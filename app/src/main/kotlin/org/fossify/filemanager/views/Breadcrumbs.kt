...
class Breadcrumbs(context: Context, attrs: AttributeSet): HorizontalScrollView(context, attrs) {
	...
	private val textColorStateList: ColorStateList
		get() = ColorStateList(arrayOf(intArrayOf(android.R.attr.state_activated), intArrayOf()), intArrayOf(accentColor, textColor))
	...
	init {
		...
		onGlobalLayout {
			stickyRootInitialLeft = if(itemsLayout.isNotEmpty())
				itemsLayout.getChildAt(0).left else 0
		}
	}
	...
	private fun handleRootStickiness(scrollX: Int) {
		if(scrollX > stickyRootInitialLeft) stickRoot(scrollX - stickyRootInitialLeft)
		else freeRoot()
	}

	private fun freeRoot() {
		if(itemsLayout.isNotEmpty()) itemsLayout.getChildAt(0).translationX = 0f
	}
	...
	private fun scrollToSelectedItem() {
		...
		if(!isFirstScroll && isShown) smoothScrollTo(scrollX, 0)
		...
		ItemBreadcrumbBinding.inflate(inflater, itemsLayout, false).apply {
			...
			breadcrumbText.setOnClickListener {v ->
				if(itemsLayout.getChildAt(index) != null && itemsLayout.getChildAt(index) == v) {
					...
					else listener?.breadcrumbClicked(index)
				}
			}
			root.tag = item
		}
	...
	private fun setupStickyBreadcrumbBackground(view: MyTextView) {
		...
		(drawable.getDrawable(0) as GradientDrawable).apply {
			setColor(context.getDialogBackgroundColor())
			setStroke(context.resources.getDimensionPixelSize(R.dimen.one_dp), context.getProperPrimaryColor().adjustAlpha(MEDIUM_ALPHA))
		}
	}
	...
	fun updateFontSize(size: Float, updateTexts: Boolean) {
		fontSize = size
		if(updateTexts) setBreadcrumb(lastPath)
	}
